...
-- 1a
select GameName from game where consoleName = 'Play Station 2';
...
-- 1c
select GameName, ReleaseDate from game where year(ReleaseDate) between '0000' and '1999'; -- all games before 2000 

-- 1d
select GameName, ReleaseDate, Series from game where ReleaseDate between '1995-01-01' and '2018-01-01';

-- 1e
select count(*) as GameCount from game where ConsoleName like 'Play Station%'; -- count of how many games

-- 1f
select GameName, ConsoleName from game where ConsoleName not like 'Play Station%';

-- 1g 
select GameName, ConsoleName, Series from game where IsOnline = true;

-- 2a
alter table game add column Developer varchar(200) not null;
alter table game add foreign key (ConsoleName) references console(ConsoleName); -- adding for updating

-- 2b - done

-- 2c - found a way to add them all, had to disable safe update for this. 
update game set Developer = (select Developer from console where ConsoleName = game.ConsoleName);

-- 2d
select Developer, GameName from game group by Developer, GameName;
